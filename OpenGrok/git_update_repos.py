...
def getDefaultConf(argv=sys.argv):
    ...
    # password of contint via get_passwd
    hostname = None
    ...
    command = "/bin/get_passwd contint %s" % hostname
    ...
    defaults['contint_passwd'] = std_out.strip()
...
def parseArgs(argv=sys.argv):
    ...
    parser.add_option("--org", action="store", type="string", dest="org",
                      help="(Mandatory) source organization from where repo will be cloned", metavar="STR")
    parser.add_option("--prefix", action="store", type="string", dest="prefix",
                      help="(Mandatory) prefix of the repositories", metavar="STR")
    parser.add_option("--rootDir", action="store", type="string", dest="rootDir",
                      help="(Mandatory) path to root dir where repos will be cloned", metavar="STR")
    parser.add_option("--username",
                      help="Username to be used for GitHub authentication. Default: " + defaults['username'],
                      default=defaults['username'])
    parser.add_option("--password", help="Password to be used for GitHub auth.", default=defaults['password'])
    parser.add_option("--contint_passwd", help="Password to be used by contint account.",
                      default=defaults['contint_passwd'])
    parser.add_option("--token", help="Token to be used for GitHub auth.", default=defaults['token'])
    parser.add_option("--url", help="URL of GitHub instance to connect. Default: " + defaults['url'],
                      default=defaults['url'])
    parser.add_option("-P", "--promptforpassword", help="Prompt for password.", action='store_true', default=False)
    parser.add_option("--updateMasters", action="store_true", dest="updateMasters",
                      help="Update all gts masters for given project (previously svn root)", default=False)
    parser.add_option("--updateActive", action="store_true", dest="updateActive",
                      help="Update all components versions of active baselines for given project (previously svn root)",
                      default=False)
    parser.add_option("--updateMigrated", action="store_true", dest="updateMigrated",
                      help="Update all components versions of migrated baselines for given project (previously svn root)",
                      default=False)
    parser.add_option("-v", "--verbose", action="store_true", dest="verbose", help="Enable verbose/debug mode",
                      default=False)
    parser.add_option("-q", "--quiet", action="store_true", dest="quiet", help="do not print anything on output",
                      default=False)
    ...
    if options.updateMasters + options.updateActive + options.updateMigrated != 1:
        parser.error(
            "You need to specify exactly one of the following options --updateMasters, --updateMigrated, --updateActive")
...
def main():
    ...
    gh = github3.GitHubEnterprise(url=options.url, username=options.username, password=options.password,
                                  token=options.token)
    logging.info("Trying to list all repos in organization: {org} with prefix {prefix}".format(org=options.org,
    ...
    if options.updateMasters:
        ...
        for repo in repos:
            if repo.name.startswith(options.prefix):
                ...
                logging.info(
                    "Repository {repository} in organization {organization} matched prefix {prefix} - clone URL: {repoURL}.".format(
                        organization=options.org, repository=repo.name, prefix=options.prefix, repoURL=remoteRepoURL))
                ...
                logging.debug(
                    "Extracting tar ball %s to %s with command: %s" % (tarFilePath, componentDirPath, command))
                p = subprocess.Popen(command.split(), stdout=subprocess.PIPE, shell=False)
                std_out, std_err = p.communicate()
                if p.returncode != 0:
        ...
        command = "sshpass,-p,%s,ssh,contint@otd2187,/sw/int/prisma/toolkit/bin/rsark.py,get_open_int_releases,--system=%s" % (
        options.contint_passwd, product)
        ...
        for baseline in baselines:
            command = "sshpass,-p,%s,ssh,contint@otd2187,/home/contint/bin/getComponentsAndVersions.sh %s %s %s" % (
            options.contint_passwd, product, baseline, "dev")
